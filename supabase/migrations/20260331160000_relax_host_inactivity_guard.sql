-- Phase 2a bugfix: relax lobby guard from capacity-based to confirmed >= 1.
-- Must ship atomically with the frontend heartbeat wiring (Phase 2b).
-- Builds on 20260331150000 which already reads v_timeout from app_config.
...
BEGIN
  ...
  -- NOTE: operates on ALL statuses, not just 'confirmed' — queued/invited users
  -- must also be moved off the dead raid.
  FOR v_entry IN
