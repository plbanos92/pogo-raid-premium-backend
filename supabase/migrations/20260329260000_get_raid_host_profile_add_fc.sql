-- Extend get_raid_host_profile to return friend_code so joiners can see the
-- host FC even when raids.friend_code was NULL at raid creation time.
-- Returns COALESCE(raids.friend_code, user_profiles.friend_code) as a live fallback.
--
-- NOTE: Must DROP first because PostgreSQL does not allow CREATE OR REPLACE to
...
BEGIN
  ...
  SELECT
    ...
    up.trainer_level,
    up.team,
    COALESCE(r.friend_code, up.friend_code)
  FROM public.raids r
